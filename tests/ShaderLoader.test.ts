...
} from "../src-js/utils/graphics/ShaderLoader";

// Mock WebGL2 context for headless testing
const createMockGL = (): Partial<WebGL2RenderingContext> => {
  const shaders = new Map<
    WebGLShader,
    { type: number; source: string; compiled: boolean }
  >();
  const programs = new Map<
    WebGLProgram,
    { linked: boolean; shaders: WebGLShader[] }
  >();
  ...
  return {
    ...
    linkProgram: jest.fn((program: WebGLProgram) => {
      ...
      if (programData) {
        ...
        const hasVertex = programData.shaders.some(
          (s) => shaders.get(s)?.type === 35633
        );
        const hasFragment = programData.shaders.some(
          (s) => shaders.get(s)?.type === 35632
        );
...
describe("ShaderLoader", () => {
  ...
  describe("loadFragment", () => {
    it("should compile valid fragment shader", () => {
      ...
      const shader = ShaderLoader.loadFragment(
    ...
    it("should cache compiled shaders", () => {
      ...
      const shader1 = ShaderLoader.loadFragment(
        mockGL as WebGL2RenderingContext,
        source
      );
      const shader2 = ShaderLoader.loadFragment(
        mockGL as WebGL2RenderingContext,
        source
      );
  ...
  describe("loadVertex", () => {
    it("should compile valid vertex shader", () => {
      const shader = ShaderLoader.loadVertex(
        mockGL as WebGL2RenderingContext,
        DEFAULT_VERTEX_SHADER
      );
  ...
  describe("cache management", () => {
    it("should clear context-specific cache", () => {
      ...
      const shader1 = ShaderLoader.loadVertex(
        mockGL as WebGL2RenderingContext,
        source
      );
      ...
      const shader2 = ShaderLoader.loadVertex(
        mockGL as WebGL2RenderingContext,
        source
      );
