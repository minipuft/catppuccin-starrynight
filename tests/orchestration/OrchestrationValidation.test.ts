...
describe('Orchestration Validation Tests', () => {
  ...
  describe('3. Event Migration System', () => {
    ...
    test('should handle multiple legacy event types', async () => {
      const { eventMigrationManager } = await import('@/core/events/EventMigrationManager');
      const { unifiedEventBus } = await import('@/core/events/UnifiedEventBus');
      ...
      // Emit various legacy events
      eventMigrationManager.emitLegacyEvent('colors-extracted', { test: 'data' });
      eventMigrationManager.emitLegacyEvent('music-sync:beat', { test: 'data' });
      eventMigrationManager.emitLegacyEvent('year3000SystemSettingsChanged', { test: 'data' });

      await new Promise(resolve => setTimeout(resolve, 20));

      // Should have received all migrated events
