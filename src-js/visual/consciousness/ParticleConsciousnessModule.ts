...
import type { BackgroundConsciousnessChoreographer } from "./BackgroundConsciousnessChoreographer";
import type { 
  ConsciousnessField, 
  BackgroundSystemParticipant 
} from "./BackgroundConsciousnessChoreographer";
import {
  BaseConsciousnessParticipant,
  ConsciousnessFieldResponder,
  ChoreographyEventResponder,
  MusicSyncUtilities,
  CSSConsciousnessIntegrator
} from "./SharedBackgroundConsciousnessUtilities";
...
export interface ParticleConsciousnessConfig {
  ...
  organicTransitionSpeed: number; // Speed of organic transitions
}
...
class SharedCanvasManager {
   ...
   */
  public getCanvas(id: string, zIndex: number = 3, mixBlendMode: string = "screen"): HTMLCanvasElement {
    ...
    Y3K?.debug?.log('SharedCanvasManager', `Created canvas: ${id} with z-index: ${zIndex}`);
    return canvas;
  }
  ...
  public removeCanvas(id: string): void {
    ...
    Y3K?.debug?.log('SharedCanvasManager', `Removed canvas: ${id}`);
  }
...
class ConsciousnessParticleRenderer {
   ...
   */
  private applyConsciousnessGlobalEffects(field: ConsciousnessField | null): void {
    if (!field || !this.ctx) return;

    // Apply consciousness-driven global alpha modulation
    const breathingAlpha = ConsciousnessFieldResponder.calculateBreathingModulation(
      0.9, field.breathingCycle, 0.1
    );
    ...
    if (field.musicalFlow.x !== 0 || field.musicalFlow.y !== 0) {
      this.ctx.translate(
        field.musicalFlow.x * 0.5, 
        field.musicalFlow.y * 0.5
      );
    }
  }
   ...
   */
  private renderHighQuality(particles: UnifiedParticle[], field: ConsciousnessField | null): void {
    if (!this.ctx) return;
    ...
    particles.forEach(particle => {
      ...
      if (particle.currentRotation !== 0) {
        const consciousRotation = field ? 
          particle.currentRotation + field.rhythmicPulse * particle.consciousnessResonance * Math.PI :
      ...
      const consciousOpacity = field ?
        ConsciousnessFieldResponder.calculateRhythmicResponse(
          particle.currentOpacity, field.rhythmicPulse, particle.flowResonance
   ...
   */
  private renderMediumQuality(particles: UnifiedParticle[], field: ConsciousnessField | null): void {
    if (!this.ctx) return;
  ...
  private getThemeParticleColor(opacity: number = 0.8): string {
    ...
    const particleGlowRgb = computedStyle.getPropertyValue('--spice-rgb-particle-glow').trim();
    if (particleGlowRgb && particleGlowRgb !== '' && !particleGlowRgb.includes('undefined')) {
      return `rgba(${particleGlowRgb}, ${opacity})`;
    }
    
    const particleCoreRgb = computedStyle.getPropertyValue('--spice-rgb-particle-core').trim();
    if (particleCoreRgb && particleCoreRgb !== '' && !particleCoreRgb.includes('undefined')) {
   ...
   */
  private renderLowQuality(particles: UnifiedParticle[], field: ConsciousnessField | null): void {
    if (!this.ctx) return;
  ...
  private renderLightweightParticle(
    ...
    if (quality === 'high') {
      ...
      const gradient = this.ctx.createRadialGradient(0, 0, 0, 0, 0, particle.currentSize);
  ...
  private renderConsciousnessParticle(
    ...
    const organicSize = ConsciousnessFieldResponder.calculateBreathingModulation(
      particle.currentSize, particle.breathingPhase, 0.2
    );
    
    // Flow-responsive color temperature
    const flowIntensity = Math.sqrt(field.musicalFlow.x ** 2 + field.musicalFlow.y ** 2);
    const colorTemp = 0.5 + flowIntensity * 0.5;
    
    if (quality === 'high') {
      // Organic gradient with consciousness colors
      const gradient = this.ctx.createRadialGradient(0, 0, 0, 0, 0, organicSize);
 ...
 */
export class ParticleConsciousnessModule extends BaseVisualSystem implements BackgroundSystemParticipant {
  ...
  constructor(
    ...
    // Initialize renderer
    this.renderer = new ConsciousnessParticleRenderer(this.canvasManager, 'sn-particle-consciousness-canvas');
    ...
    Y3K?.debug?.log('ParticleConsciousnessModule', 'Unified particle system initialized');
  }
  ...
  public override async _performSystemSpecificInitialization(): Promise<void> {
    ...
    try {
      ...
      Y3K?.debug?.log('ParticleConsciousnessModule', 'Particle consciousness system initialized successfully');
      
    } catch (error) {
      Y3K?.debug?.error('ParticleConsciousnessModule', 'Failed to initialize particle consciousness system:', error);
    }
  }
  ...
  private createPerformanceConfig(): ParticlePerformanceConfig {
    ...
    const capabilities = this.consciousnessChoreographer?.getCurrentConsciousnessField()?.deviceCapabilities || {
      memoryGB: 4,
      cpuCores: 4,
  ...
  private initializeParticlePool(): void {
    this.particlePool = [];
    const totalParticles = this.performanceConfig.maxParticles + this.performanceConfig.maxFieldParticles;
    
    for (let i = 0; i < totalParticles; i++) {
      this.particlePool.push(this.createPooledParticle());
    }
    
    Y3K?.debug?.log('ParticleConsciousnessModule', `Initialized particle pool with ${totalParticles} particles`);
  }
  ...
  private handleBeatEvent(intensity: number): void {
    ...
    this.activeParticles.forEach(particle => {
      if (particle.active) {
        ...
        const pulseMultiplier = 1 + intensity * particle.consciousnessResonance * 0.5;
        particle.targetSize = Math.min(particle.targetSize * pulseMultiplier, 20);
        
        // Apply beat influence to opacity
        particle.targetOpacity = Math.min(particle.targetOpacity + intensity * 0.2, 1);
      }
    });
  }
  ...
  private registerWithConsciousnessChoreographer(): void {
    if (!this.consciousnessChoreographer) {
      Y3K?.debug?.log('ParticleConsciousnessModule', 'Consciousness choreographer not available, skipping registration');
      return;
    }
    
    try {
      this.consciousnessChoreographer.registerConsciousnessParticipant(this);
      Y3K?.debug?.log('ParticleConsciousnessModule', 'Successfully registered with consciousness choreographer');
    } catch (error) {
      Y3K?.debug?.error('ParticleConsciousnessModule', 'Failed to register with consciousness choreographer:', error);
    }
  }
   ...
   */
  private spawnParticle(mode: ParticleMode, intensity: number): UnifiedParticle | null {
   ...
   */
  private initializeLightweightParticle(particle: UnifiedParticle, intensity: number): void {
   ...
   */
  private initializeFieldParticle(particle: UnifiedParticle, intensity: number): void {
   ...
   */
  private initializeHybridParticle(particle: UnifiedParticle, intensity: number): void {
    // Initialize both lightweight and field properties
    this.initializeLightweightParticle(particle, intensity);
    this.initializeFieldParticle(particle, intensity * 0.8);
    
    // Blend characteristics
    particle.currentSize = (particle.currentSize + (particle.fieldProps?.baseSize || 2)) * 0.5;
    particle.targetSize = (particle.targetSize + (particle.fieldProps?.baseSize || 2) * 1.5) * 0.5;
   ...
   */
  private initializeConsciousnessParticle(particle: UnifiedParticle, intensity: number): void {
  ...
  private updatePerformanceMetrics(deltaTime: number): void {
    ...
    if (this.lastFrameTime > 0) {
      ...
      if (this.averageFrameTime > 20 && this.performanceConfig.renderQuality !== 'low') {
        this.performanceConfig.renderQuality = 'medium';
        Y3K?.debug?.log('ParticleConsciousnessModule', 'Reduced render quality due to performance');
      } else if (this.averageFrameTime > 25) {
        this.performanceConfig.renderQuality = 'low';
        Y3K?.debug?.log('ParticleConsciousnessModule', 'Switched to low quality rendering');
      } else if (this.averageFrameTime < 16 && this.performanceConfig.renderQuality !== 'high') {
   ...
   */
  private applyConsciousnessInfluence(particle: UnifiedParticle, deltaTime: number): void {
    if (!this.currentConsciousnessField || !this.consciousnessConfig.enableOrganicFlow) return;
    ...
    if (this.consciousnessConfig.enableBreathingSynchronization) {
      const breathingInfluence = ConsciousnessFieldResponder.calculateBreathingModulation(
        1.0, field.breathingCycle + particle.breathingPhase, 0.1
      );
    ...
    if (this.consciousnessConfig.enableCellularGrowth) {
      const growthPhase = particle.organicGrowthPhase + field.cellularGrowthRate;
    ...
    const rhythmicInfluence = ConsciousnessFieldResponder.calculateRhythmicResponse(
      1.0, field.rhythmicPulse, particle.consciousnessResonance
    );
   ...
   */
  private updateParticleTransitions(particle: UnifiedParticle, deltaTime: number): void {
    const transitionSpeed = this.consciousnessConfig.organicTransitionSpeed * 0.002;
  ...
  public getConsciousnessContribution(): any {
    return {
      particleDensity: this.activeParticles.length / this.performanceConfig.maxParticles,
      energyContribution: this.averageParticleEnergy(),
      organicFlowContribution: this.calculateOrganicFlowContribution(),
      renderingLoad: this.performanceConfig.renderQuality === 'high' ? 1.0 : 
                    this.performanceConfig.renderQuality === 'medium' ? 0.6 : 0.3
    };
  }
  ...
  public onConsciousnessFieldUpdate(field: ConsciousnessField): void {
    try {
      ...
      Y3K?.debug?.log('ParticleConsciousnessModule', 'Updated from consciousness field:', {
      ...
      Y3K?.debug?.error('ParticleConsciousnessModule', 'Error updating from consciousness field:', error);
    }
  }
  ...
  public onChoreographyEvent(eventType: string, payload: any): void {
    try {
      ...
      Y3K?.debug?.log('ParticleConsciousnessModule', `Handled choreography event: ${eventType}`, payload);
    } catch (error) {
      Y3K?.debug?.error('ParticleConsciousnessModule', `Error handling choreography event ${eventType}:`, error);
    }
  }
  ...
  private adaptToConsciousnessField(field: ConsciousnessField): void {
    ...
    this.activeParticles.forEach(particle => {
      if (particle.active) {
        ...
        particle.consciousnessResonance = Math.min(1.0, 
  ...
  private handleEnergySurge(payload: any): void {
    ...
    this.activeParticles.forEach(particle => {
      if (particle.active) {
        particle.targetSize *= ChoreographyEventResponder.handleEnergySurge(1.0, surgeIntensity);
        particle.targetOpacity = Math.min(1.0, particle.targetOpacity + surgeIntensity * 0.3);
      }
    });
  }
  ...
  private handleBreathingCycle(payload: any): void {
    ...
    this.activeParticles.forEach(particle => {
      if (particle.active) {
        particle.breathingPhase = breathingPhase + particle.consciousnessResonance * Math.PI;
  ...
  private handleCellularGrowth(payload: any): void {
    ...
    this.activeParticles.forEach(particle => {
      if (particle.active) {
        ...
        const growthInfluence = Math.sin(particle.organicGrowthPhase) * 0.2 + 1.0;
        particle.targetSize *= growthInfluence;
      }
    });
  }
  ...
  private handlePerformanceAdapt(payload: any): void {
    ...
    if (newMode.qualityLevel < 0.3) {
      this.performanceConfig.renderQuality = 'low';
      this.performanceConfig.maxParticles = Math.floor(this.performanceConfig.maxParticles * 0.5);
    } else if (newMode.qualityLevel < 0.7) {
      this.performanceConfig.renderQuality = 'medium';
      this.performanceConfig.maxParticles = Math.floor(this.performanceConfig.maxParticles * 0.8);
    } else {
      this.performanceConfig.renderQuality = 'high';
    }
    ...
    Y3K?.debug?.log('ParticleConsciousnessModule', `Adapted to performance mode: ${newMode.name}`, {
  ...
  private averageParticleEnergy(): number {
    ...
    const totalEnergy = this.activeParticles.reduce((sum, particle) => {
      return sum + (particle.currentSize * particle.currentOpacity * particle.consciousnessResonance);
    }, 0);
  ...
  private calculateOrganicFlowContribution(): { x: number; y: number } {
    ...
    const flowVector = this.activeParticles.reduce((sum, particle) => {
      return {
  ...
  public async healthCheck(): Promise<HealthCheckResult> {
    ...
    return {
      healthy: isHealthy,
      ok: isHealthy,
      details: `Particles: ${activeCount}/${this.performanceConfig.maxParticles}, ` +
  ...
  public override _performSystemSpecificCleanup(): void {
    ...
    if (this.consciousnessChoreographer) {
      try {
        this.consciousnessChoreographer.unregisterConsciousnessParticipant('ParticleConsciousnessModule');
        Y3K?.debug?.log('ParticleConsciousnessModule', 'Unregistered from consciousness choreographer');
      } catch (error) {
        Y3K?.debug?.error('ParticleConsciousnessModule', 'Error unregistering from consciousness choreographer:', error);
      }
    }
    ...
    Y3K?.debug?.log('ParticleConsciousnessModule', 'System cleanup completed');
  }
  ...
  public spawnParticleCompat(
    ...
    // Map to new API
    const mode: ParticleMode = energy > 0.7 ? 'consciousness' : 
                              intensity > 0.6 ? 'hybrid' : 'lightweight';
  ...
  public setParticleDensity(density: number): void {
    ...
    this.performanceConfig.maxParticles = Math.max(10, Math.min(300, maxParticles));
  }
