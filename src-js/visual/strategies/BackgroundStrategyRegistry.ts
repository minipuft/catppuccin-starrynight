...
export class BackgroundStrategyRegistry implements IColorStrategyRegistry {
  ...
  selectStrategy(criteria: StrategySelectionCriteria): IColorProcessor | null {
    const availableStrategies = Array.from(this.strategiesMap.values())
      ...
      .sort((a, b) => this.scoreStrategy(b, criteria) - this.scoreStrategy(a, criteria));
   ...
   */
  private scoreStrategy(registration: StrategyRegistration, criteria: StrategySelectionCriteria): number {
    let score = 0;
    ...
    if (criteria.deviceCapabilities) {
      ...
      if (criteria.deviceCapabilities.memoryMB && criteria.deviceCapabilities.memoryMB < 4000) {
    ...
    if (criteria.userPreferences) {
      if (criteria.userPreferences.enableAdvancedBlending && 
   ...
   */
  private inferStrategyMetadata(strategy: IColorProcessor): StrategyRegistration['metadata'] {
    const strategyName = strategy.getStrategyName();
    const tags: string[] = [];
    let category: 'foundation' | 'enhancement' | 'accent' | 'effects' = 'enhancement';
    let priority = 5;
   ...
   */
  recordStrategyProcessingTime(strategyName: string, processingTime: number): void {
    const registration = this.strategiesMap.get(strategyName);
    if (registration) {
    ...
    try {
      for (const [name, registration] of this.strategiesMap.entries()) {
        try {
          ...
          if ('healthCheck' in registration.strategy && 
              typeof registration.strategy.healthCheck === 'function') {
            
            const healthResult = await (registration.strategy as any).healthCheck();
   ...
   */
  getAllStrategyInfo(): Array<{ name: string; registration: StrategyRegistration }> {
