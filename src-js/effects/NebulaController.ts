// ============================================================================
// 🎆 NEBULA CONTROLLER – Phase 2 of Cosmic Mood Field
// ============================================================================
...
import { UserGenreHistory } from "@/store/UserHistory";
...
export class NebulaController {
  ...
  private _recordDuration(start: number): void {
    ...
    if (this.frameDurations.length > NebulaController.FRAME_HISTORY) {
    ...
    this.perf?.shouldUpdate("nebulaCtr", 1000) &&
      this.perf?.endTiming("NebulaController", start);
  }
...
export function initializeNebulaController(
  y3k: Year3000System | null = null
): NebulaController {
  // Singleton guard for hot-reloads.
  const g = globalThis as any;
  if (g.__SN_nebulaController)
    return g.__SN_nebulaController as NebulaController;

  const instance = new NebulaController(y3k);
  g.__SN_nebulaController = instance;
  return instance;
}
