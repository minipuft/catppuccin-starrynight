 ...
 */
export abstract class ServiceSystemBase implements IManagedSystem, IServiceAwareSystem {
  ...
  protected _legacyPerformanceAnalyzer: SimplePerformanceCoordinator | null = null;
  ...
  constructor(config: AdvancedSystemConfig | Year3000Config = ADVANCED_SYSTEM_CONFIG) {
    ...
    Y3KDebug?.debug?.log("ServiceSystemBase", `Created ${this.systemName} with service composition`);
  }
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    try {
      ...
      const isHealthy = this.initialized && !this.destroyed && systemSpecificHealth.healthy;
      
      return {
        healthy: isHealthy,
        ok: isHealthy,
        system: this.systemName,
        details: systemSpecificHealth.details || `${this.systemName} system health check`,
  ...
  destroy(): void {
    if (!this.services.lifecycle) {
      Y3KDebug?.debug?.warn("ServiceSystemBase", `${this.systemName}: Lifecycle service not available for cleanup`);
      return;
    }
  ...
  forceRepaint?(reason?: string): void {
    Y3KDebug?.debug?.log("ServiceSystemBase", `${this.systemName} force repaint: ${reason || 'no reason'}`);
  ...
  protected trackPerformance<T>(operationName: string, operation: () => T): T {
    ...
    return this.services.performance.trackOperation(this.systemName, operationName, operation);
  }
  
  /**
   * Track async operation performance (legacy compatible)
   */
  protected async trackPerformanceAsync<T>(operationName: string, operation: () => Promise<T>): Promise<T> {
    if (!this.services.performance) return await operation();
    
    return await this.services.performance.trackOperationAsync(this.systemName, operationName, operation);
  }
  
  /**
   * Subscribe to events (legacy compatible)
   */
  protected subscribeToEvent<T>(eventName: string, handler: (data: T) => void): void {
    this.services.events?.subscribe(this.systemName, eventName, handler);
  }
...
export abstract class ServiceVisualSystemBase extends ServiceSystemBase {
   ...
   */
  protected resizeCanvas(canvasId: string, width: number, height: number): boolean {
    return this.services.canvas?.resizeCanvas(this.systemName, canvasId, width, height) || false;
  }
  ...
  protected handleSettingsChange(event: Event): void {
    ...
    Y3KDebug?.debug?.log("ServiceVisualSystemBase", 
