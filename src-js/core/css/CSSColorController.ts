/**
 * CSSColorController - Unified Color State Coordination
 *
...
export class CSSColorController implements IManagedSystem {
  ...
  public async initialize(): Promise<void> {
    ...
    this.settingsUnsubscribe = settings.onChange((event) =>
      void this.handleSettingsChange(event)
    );
    ...
    console.log("🎨 [CSSColorController] Initialized successfully");
  }
  ...
  public destroy(): void {
    ...
    unifiedEventBus.unsubscribeAll("CSSColorController");
  ...
  private getCurrentConfig(): ColorStateConfig {
    // Determine active flavor based on current palette system
    const paletteSystem = paletteSystemManager.getCurrentPaletteSystem();

    const currentFlavor: UnifiedFlavor =
      paletteSystem === "year3000"
        ? paletteSystemManager.getCurrentDefaultFlavor()
        : ((settings.get("catppuccin-flavor") as UnifiedFlavor) ||
            paletteSystemManager.getCurrentDefaultFlavor());
    ...
    return {
      ...
      accentColor: settings.get("catppuccin-accentColor") as
        | UnifiedColorName
        | "dynamic",
      textAccentDynamic: settings.get("sn-dynamic-text-accent"),
  ...
  private async applyColorStateToCSSVariables(
    ...
    const cssUpdates = {
      ...
      "--sn-text-accent-mode": `"${
        state.effectiveConfig.textAccentDynamic ? "dynamic" : "static"
      }"`,
  ...
  private async writePaletteColorsToCSS(
    ...
    if (!flavorPalette) {
      console.warn(
        `[CSSColorController] No palette found for flavor: ${state.effectiveConfig.paletteSystemFlavor}`
      );
      return;
    }
    ...
    console.log(
      `🎨 [CSSColorController] Applied Year3000 palette themed colors`,
      {
  ...
  /**
   * 🔧 PHASE 2: Generic method for other systems to queue CSS updates through CSSColorController using coordination
   * This makes CSSColorController the single CSS authority for all color-related variables
   */
    ...
    for (const varName of criticalVars) {
      if (cssUpdates[varName]) {
        ...
        if (computedValue !== expectedValue) {
          console.warn(
            `🎨 [CSSColorController] Variable verification failed: ${varName} = "${computedValue}" (expected "${expectedValue}")`
          );
          return false;
        }
      }
  ...
  public async updateColorState(
    ...
    try {
      ...
      if (hasChanged) {
        ...
        console.log(
          `🎨 [CSSColorController] Color state updated (${trigger}):`,
          {
  ...
  private applyBrightnessModeAttribute(
    ...
    // Log for debugging
    console.log(
      `🎨 [CSSColorController] Brightness mode set: ${brightnessMode} (CSS-only, no JS processing)`
    );
  }
  ...
  private async handleSettingsChange(event: SettingsChangeEvent): Promise<void> {
    ...
    if (
      ...
      else if (settingKey === "sn-dynamic-text-accent") trigger = "accent";
    ...
    Object.entries(processedColors).forEach(([key, value]) => {
      ...
      if (shouldGenerateRGB) {
        try {
          ...
          console.warn(
            `[CSSColorController] Failed to convert ${key} to RGB:`,
            error
          );
        }
      }
  ...
  private async handleProcessedColors(event: any): Promise<void> {
    ...
    // CSSColorController is the SINGLE source of truth for ALL CSS variable writes
   ...
   * This makes CSSColorController the single CSS authority for all systems
   */
  ...
  private async writeDynamicPaletteColors(
    ...
    try {
      ...
      if (ADVANCED_SYSTEM_CONFIG.enableDebug) {
        console.log(`🎨 [CSSColorController] Dynamic palette generated:`, {
      ...
      console.error(
        "[CSSColorController] Dynamic palette generation failed:",
        error
      );
    }
  }
...
// Global instance
export const globalCSSColorController = new CSSColorController();