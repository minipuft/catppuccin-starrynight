/**
 * GradientPerformanceOptimizer - Performance Strategy for Complex Gradient Animations
...
/**
 * GradientPerformanceOptimizer manages performance for complex gradient systems
...
export class GradientPerformanceOptimizer {
  ...
  private optimizationCallbacks: Map<
    string,
    (quality: QualitySettings) => void
  > = new Map();
  ...
  constructor(
    ...
    const cssController = getGlobalCSSVariableWriterSafe();
    if (cssController) {
      this.cssVariableManager = cssController;
    } else {
      Y3KDebug?.debug?.warn(
        "GradientPerformanceOptimizer",
        "CSSVariableWriter not available, CSS variable management disabled"
      );
  ...
  private updatePerformanceMetrics(): void {
    ...
    this.currentMetrics.renderTime =
      this.performanceAnalyzer.getAverageTime?.("render") || 0;

    // Update frame drops
    const currentFrameDrops =
      this.performanceAnalyzer.getMedianFPS?.() < 30 ? 1 : 0; // Estimate frame drops based on FPS
  ...
  private detectThermalThrottling(): void {
    ...
    if (this.currentMetrics.fps < 30 && this.currentMetrics.renderTime > 50) {
      ...
      this.currentMetrics.fps < 40 &&
      this.currentMetrics.renderTime > 30
    ) {
      this.currentMetrics.thermalState = "serious";
    } else if (
      this.currentMetrics.fps < 50 &&
      this.currentMetrics.renderTime > 20
    ) {
  ...
  private handleThermalStateChange(): void {
    ...
    Y3KDebug?.debug?.log(
      "GradientPerformanceOptimizer",
      `Thermal state changed to: ${thermalState}`
    );
  }
  ...
  private analyzePerformance(): void {
    ...
    if (this.currentMetrics.fps < this.adaptiveThresholds.fpsError) {
      ...
      actions.push({
        ...
        reason: `FPS below warning threshold: ${this.currentMetrics.fps.toFixed(
          1
        )}`,
        priority: 2,
        timestamp: currentTime,
    ...
    if (this.currentMetrics.memoryUsage > this.adaptiveThresholds.memoryError) {
      actions.push({
        ...
        reason: `Memory usage critical: ${this.currentMetrics.memoryUsage.toFixed(
          1
        )}MB`,
        priority: 1,
        timestamp: currentTime,
      ...
      actions.push({
        type: "optimize",
        system: "depth-layers",
        reason: `Memory usage warning: ${this.currentMetrics.memoryUsage.toFixed(
          1
        )}MB`,
        priority: 2,
        timestamp: currentTime,
    ...
    // Check render time
    if (
      this.currentMetrics.renderTime > this.adaptiveThresholds.renderTimeError
    ) {
      actions.push({
        type: "reduce",
        system: "animation-speed",
        reason: `Render time critical: ${this.currentMetrics.renderTime.toFixed(
          1
        )}ms`,
        priority: 1,
        timestamp: currentTime,
  ...
  private executePerformanceActions(actions: PerformanceAction[]): void {
    ...
    actions.forEach((action) => {
      ...
      Y3KDebug?.debug?.log(
        "GradientPerformanceOptimizer",
        `Performance action: ${action.type} ${action.system} - ${action.reason}`
      );
    });
  ...
  private disableSystem(systemName: string): void {
    switch (systemName) {
      case "fluid-gradient":
        this.qualitySettings.fluidGradientEnabled = false;
        break;
  ...
  private reduceSystemQuality(systemName: string): void {
    switch (systemName) {
      ...
      case "depth-layers":
        this.qualitySettings.depthLayers = Math.max(
          1,
      ...
      case "animation-speed":
        this.qualitySettings.animationSpeed = Math.max(
          0.2,
          this.qualitySettings.animationSpeed - 0.1
        );
        break;

      case "parallax-strength":
        this.qualitySettings.parallaxStrength = Math.max(
          0.1,
  ...
  private reduceQualitySettings(): void {
    ...
    if (currentIndex > 0) {
      const newLevel = qualityLevels[
        currentIndex - 1
      ] as QualitySettings["level"];
  ...
  private updatePerformanceVariables(): void {
    ...
    const controller = this.cssVariableManager;
    if (controller) {
  ...
  private notifyQualityChange(): void {
    ...
    document.dispatchEvent(
      new CustomEvent("performance:quality-changed", {
        detail: {
  ...
  private setQualityLevel(level: QualitySettings["level"]): void {
    ...
    switch (level) {
      ...
      case "medium":
        this.qualitySettings.fluidGradientEnabled = true;
  ...
  public registerOptimizationCallback(
    systemName: string,
    callback: (quality: QualitySettings) => void
  ): void {
  ...
  public isPerformanceOptimal(): boolean {
    return (
      this.currentMetrics.fps >= this.adaptiveThresholds.fpsWarning &&
      this.currentMetrics.memoryUsage <=
        this.adaptiveThresholds.memoryWarning &&
      this.currentMetrics.renderTime <=
        this.adaptiveThresholds.renderTimeWarning
    );
  }
  ...
  public destroy(): void {
    ...
    Y3KDebug?.debug?.log(
      "GradientPerformanceOptimizer",
      "Performance optimizer destroyed"
    );
  }
}
