...
import { VisualSystemFacade, VisualSystemKey } from "@/visual/integration/VisualSystemFacade";
import { NonVisualSystemFacade, NonVisualSystemKey } from "@/core/integration/NonVisualSystemFacade";
...
export class SystemCoordinator {
  ...
  public async getNonVisualSystem<T = any>(key: NonVisualSystemKey): Promise<T | null> {
    if (!this.nonVisualFacade) return null;
    return await this.nonVisualFacade.getSystem<T>(key);
  }

  public async getSystem<T = any>(key: VisualSystemKey | NonVisualSystemKey): Promise<T | null> {
    ...
    if (this.nonVisualFacade) {
      try {
        return await this.nonVisualFacade.getSystem<T>(key as NonVisualSystemKey);
      } catch (error) {
