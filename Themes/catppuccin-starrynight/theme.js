...
(() => {
  ...
  var init_ColorHarmonyEngine = __esm({
    "src-js/systems/ColorHarmonyEngine.ts"() {
      ...
      ColorHarmonyEngine = class extends BaseVisualSystem {
    ...
    "src-js/systems/nowPlaying/NowPlayingCoordinator.ts"() {
      ...
      _NowPlayingCoordinator = class _NowPlayingCoordinator {
        constructor(config = {}) {
          ...
          if (this.config.enableDebug) {
            console.log("\u{1F3B5} [NowPlayingCoordinator] Initialized with RAF-based batching");
          }
        }
        /**
        ...
        queueUpdate(property, value) {
          ...
          if (this.config.enableDebug && this.pendingUpdates.size === 1) {
            console.log(`\u{1F3B5} [NowPlayingCoordinator] Queuing first update: ${property}`);
          }
          this.scheduleFlush();
        }
        /**
        ...
        flushUpdates() {
          ...
          if (this.config.enableDebug) {
            console.log(`\u{1F3B5} [NowPlayingCoordinator] Flushing ${updateCount} updates atomically`);
          }
          if (updateCount > (this.config.maxBatchSize || 50)) {
            console.warn(`\u{1F3B5} [NowPlayingCoordinator] Large batch detected (${updateCount} updates), may impact performance`);
          }
          for (const update of this.pendingUpdates.values()) {
            try {
              root.style.setProperty(update.property, update.value);
            } catch (error) {
              console.error(`\u{1F3B5} [NowPlayingCoordinator] Failed to apply ${update.property}:`, error);
            }
          }
          ...
          if (this.performanceAnalyzer) {
            this.performanceAnalyzer.emitTrace?.(`[NowPlayingCoordinator] Flushed ${updateCount} updates in ${flushTime.toFixed(2)}ms`);
          }
          if (this.config.enableDebug && flushTime > 5) {
            console.warn(`\u{1F3B5} [NowPlayingCoordinator] Slow flush detected: ${flushTime.toFixed(2)}ms for ${updateCount} updates`);
          }
        }
        /**
        ...
        setupDOMObservation() {
          ...
          this.nowPlayingElement = document.querySelector(MODERN_SELECTORS.nowPlayingBar);
          if (!this.nowPlayingElement) {
            if (this.config.enableDebug) {
              console.warn("\u{1F3B5} [NowPlayingCoordinator] Now playing bar not found, deferring DOM observation");
            }
          ...
          this.domObserver = new MutationObserver(() => {
            ...
            if (this.config.enableDebug) {
              console.log("\u{1F3B5} [NowPlayingCoordinator] DOM change detected, forcing refresh");
            }
          });
          ...
          if (this.config.enableDebug) {
            console.log("\u{1F3B5} [NowPlayingCoordinator] DOM observation active on now playing bar");
          }
        }
        /**
        ...
        destroy() {
          ...
          if (this.config.enableDebug) {
            ...
            console.log(`\u{1F3B5} [NowPlayingCoordinator] Destroyed. Performance: ${this.flushCount} flushes, ${avgFlushTime.toFixed(2)}ms avg`);
          }
        }
      };
      _NowPlayingCoordinator.instance = null;
